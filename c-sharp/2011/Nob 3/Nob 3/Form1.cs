...
namespace Nob_3
{
    public partial class Form1 : Form
    {
        ...
        {
            Words = textBox1.Text;
            Thread t = new Thread(new ThreadStart(_4sharedGetSongsByAnyWord));
            t.Start();
            //MessageBox.Show(site._4sharedGetSongsByAnyWord("dire straits").ToString());
        }
        public delegate void UpdateTextCallback(int num);
...
*/
        ...
        public int ParseHTMLForSongsURLS(string URL)
        {
            ...
            foreach (Match match in matches)
            {
                if (match.Groups["SONG_URL"] != null && match.Groups["SIZE"] != null && match.Groups["TITLE"] != null && match.Groups["ARTIST"] != null)
                {
                    ...
                    list.BeginInvoke(new UpdateTextCallback(AddNewItemByIndex), new object[] { IndexAdding });
        ...
        public void AddNewItemByIndex(int Index)
        {
            ListViewItem lvi = list.Items.Add(ListOfSongs.Artist[Index]);
            lvi.UseItemStyleForSubItems = false;
            lvi.SubItems.Add(ListOfSongs.Song[Index]);
            lvi.SubItems.Add(ListOfSongs.URL[Index]);
            lvi.SubItems.Add(ListOfSongs.Size[Index]);
